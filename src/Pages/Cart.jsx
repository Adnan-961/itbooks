...
const Cart = () => {
  ...
  const Fav = JSON.parse(localStorage.getItem("favBooks"));
  let totalPrice = 0;
  for (let i = 0; i < Fav.length; i++) {
    totalPrice += parseFloat(Fav[i].price.slice(1));
  }
  let taxedPrice = totalPrice * 0.15;
    ...
    <>
      <Fade>
        ...
        <div className="cart__container">
          <div className="inner__cart__cart_container">
            ...
            {!JSON.stringify(Fav) === "[]" && (
              <div className="Final__price">
                <div className="price__container">
                  ...
                  <div>
                    <span>15% Tax:</span>
                    <span>${parseFloat(taxedPrice).toFixed(2)}</span>
                  </div>
                  <div>
                    <span className="bold">Total:</span>
                    <span className="bold">
                      ${parseFloat(totalPrice + totalPrice * 0.15).toFixed(2)}
                    </span>
                  </div>
                  <div>
                    <button
                      id="checkout"
                      onClick={() => toast.info("Thank you!")}
                    >
                      Checkout
                    </button>
                  </div>
...
function CartItems({ title, image, price, isbn13 }) {
  const Fav = JSON.parse(localStorage.getItem("favBooks"));
  return (
    <>
      <ToastContainer
      ...
      <div className="info__container">
        <div className="cart__book">
          <img src={image} alt="book" className="cart__book__image" />
          <div className="cart__book__info">
            ...
            <h5
              ...
              onClick={() => {
                const NewFav = Fav.filter((item) => item.isbn13 !== isbn13);
                localStorage.setItem("favBooks", JSON.stringify(NewFav));
        ...
        <div className="cart__prices">
          <h3>
            {price === "$0.00"
              ...
              : "$" +
                (
                  parseFloat(parseFloat(price.slice(1))) +
                  parseFloat(price.slice(1) * 0.15)
                ).toFixed(2)}
          </h3>
        </div>
      </div>
